...
public class VisualChessBoard extends JLayeredPane {
	...
	public VisualChessBoard() {
		...
		String k=JOptionPane.showInputDialog("Two Player or CPU");
		if(k.equals("CPU")) {
			String m = JOptionPane.showInputDialog("Easy,Medium,or Hard");
			cpu = new AI(m,Color.BLACK);
		}
		else {
			player2=Color.BLACK;
		}
	...
	private class MoveManager extends MouseAdapter {
		...
		public void mouseReleased(MouseEvent e) {
			...
			int retVal = logicalBoard.isLegalMove(origGridX, origGridY, gridX, gridY,player1);
			if (retVal < 0) {
				// illegal moves
				...
				if (retVal == -4) {
					...
					statusLabel.setText("CPU move not yours");
				}
				else if (retVal == -3) {
					// returned the piece to original location
					statusLabel.setText ("");
